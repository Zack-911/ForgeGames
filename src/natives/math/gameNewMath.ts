...
function generateMath(diff: string): { question: string; answer: number; points: number } {
  ...
  // medium
  const type = rand(0, 2)
  if (type === 0) { const a = rand(2, 12), b = rand(2, 12); return { question: `${a} × ${b}`, answer: a * b, points: 200 } }
  if (type === 1) { const b = rand(2, 9), a = b * rand(2, 12); return { question: `${a} ÷ ${b}`, answer: a / b, points: 200 } }
