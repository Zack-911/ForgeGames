...
export default new NativeFunction({
  ...
  args: [
    ...
    { name: 'opponentID', description: 'Opponent user (omit for bot play)', type: ArgType.User,    required: false, rest: false },
  ],
  output: ArgType.Json,
  execute(ctx, [guild, channel, opponent]) {
    ...
    if ((session.type as string) !== 'rps')   return this.customError('This session is not an RPS game.')
