...
export interface TriviaQuestion {
  question: string
  answer: string // correct answer (lowercase for comparison)
...
export function getQuestion(opts: {
  ...
  if (opts.exclude?.length) pool = pool.filter((q) => !opts.exclude!.includes(q.question))
  if (!pool.length) return null
  return pool[Math.floor(Math.random() * pool.length)]!
}

export function shuffleChoices(q: TriviaQuestion): string[] {
